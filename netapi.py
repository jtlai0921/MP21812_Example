    ...
    def send_file(self, path):
        ...
        for tag, sendAction in fileInfo:
            ...
            try:
                ...
                logging.error('Exception when send action: %s %s', tag, e.args[0])
                backTag = e.args[0]
            except Exception as e:
...
def save_file(fileInfo, target):
    ...
    if not fileName:
        ...
        return False
    if not fileSize:
        logging.debug('invalid file size: %d', fileSize)
        return False
    if content or tempFile:
        fullName = os.path.join(target, fileName)
        dirname  = os.path.dirname(fullName)
        if not os.path.exists(dirname):
            logging.debug('create directory: %s', dirname)
            os.makedirs(dirname)
        if content:
            ...
            try:
                ...
                logging.error('Exception when save content to %s: %s', fullName, str(e))
                return False
        else:
